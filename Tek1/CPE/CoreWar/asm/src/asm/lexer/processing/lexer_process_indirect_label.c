...
** lexer_process_indirect_label.c
** File description:
** lexer_process_indirect_label.c
*/
...
bool lexer_process_indirect_label(lexer_t *lexer, reader_t *reader)
{
    ...
    if (string_get_length(str) == 1) {
        lexer_error(lexer, reader, "invalid indirect label");
        return false;
    }
    lexer_push_token(lexer, reader, TOKEN_INDIRECT_LABEL, str);
    return true;
}
