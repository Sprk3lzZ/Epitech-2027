...
{
  id=$1
  $CAT ./tests/minishell_tests | $GREP -A1000 "\[$id\]" | $GREP -B1000 "^\[$id-END\]" | $GREP -v "^\[.*\]"
}
...
load_test()
{
  ...
  then
    ...
    if [ $debug -ge 1 ]
    then
      echo -e "Test $id ($NAME) :$RED KO $RESET- Check output in /tmp/minishell_tests/$id/"
      TESTS_KO=`$EXPR $TESTS_KO + 1`
      TESTS_KO_LIST="$TESTS_KO_LIST $id"
      EXIT_CODE=1
      $MKDIR -p /tmp/minishell_tests/$id 2>/dev/null
      $CP /tmp/.shell.$$ /tmp/minishell_tests/$id/mysh.out
      $CP /tmp/.refer.$$ /tmp/minishell_tests/$id/tcsh.out
    else
      echo "KO"
    fi
  fi
...
if [ $# -eq 0 ]
then
  for lst in `cat ./tests/minishell_tests | grep "^\[.*\]$" | grep -vi end | sed s/'\['// | sed s/'\]'//`
  do
    path_backup=$PATH
    load_test $lst 1
    export PATH=$path_backup
  done
    echo -e "\n==============\nSummary : $GREEN$TESTS_OK tests OK$RESET, $RED$TESTS_KO tests KO$RED$RESET"
    echo -e "List of KO tests : $PURPLE$TESTS_KO_LIST$RESET\n=============="
else
  if [ $# -eq 1 ]
  then
    load_test $1 0
